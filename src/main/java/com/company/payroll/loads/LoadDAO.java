...
public class LoadDAO {
    ...
    public int add(Load load) {
        logger.info("Adding new load - Number: {}, Customer: {}, Customer2: {}, Driver: {}, Amount: ${}", 
            ...
            load.getGrossAmount());
        String sql = """
            INSERT INTO loads (load_number, po_number, customer, customer2, pick_up_location, drop_location, 
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
            ...
            ps.setString(5, load.getPickUpLocation());
            ps.setString(6, load.getDropLocation());
            ps.setObject(7, load.getDriver() != null ? load.getDriver().getId() : null);
            ps.setString(8, load.getTruckUnitSnapshot());
            ps.setInt(9, load.getTrailerId());
            ps.setString(10, load.getTrailerNumber());
            ps.setString(11, load.getStatus().name());
            ps.setDouble(12, load.getGrossAmount());
            ps.setString(13, load.getNotes());
            if (load.getPickUpDate() != null)
                ps.setDate(14, java.sql.Date.valueOf(load.getPickUpDate()));
            else
                ps.setNull(14, java.sql.Types.DATE);
            if (load.getPickUpTime() != null)
                ps.setTime(15, java.sql.Time.valueOf(load.getPickUpTime()));
            else
                ps.setNull(15, java.sql.Types.TIME);
            if (load.getDeliveryDate() != null)
                ps.setDate(16, java.sql.Date.valueOf(load.getDeliveryDate()));
            else
                ps.setNull(16, java.sql.Types.DATE);
            if (load.getDeliveryTime() != null)
                ps.setTime(17, java.sql.Time.valueOf(load.getDeliveryTime()));
            else
                ps.setNull(17, java.sql.Types.TIME);
            ps.setString(18, load.getReminder());
            ps.setInt(19, load.isHasLumper() ? 1 : 0);
            ps.setInt(20, load.isHasRevisedRateConfirmation() ? 1 : 0);
            ps.executeUpdate();
            ResultSet keys = ps.getGeneratedKeys();
            if (keys.next()) {
                int id = keys.getInt(1);
                logger.info("Load added successfully with ID: {}", id);
                // Save customers if not exists
    ...
    public void update(Load load) {
        ...
        String sql = """
            UPDATE loads SET load_number=?, po_number=?, customer=?, customer2=?, pick_up_location=?, 
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL)) {
            ...
            String sql = """
                ...
                AND (
                    (delivery_date IS NOT NULL AND delivery_date >= ? AND delivery_date <= ?)
                    OR (delivery_date IS NULL AND pickup_date IS NOT NULL AND pickup_date >= ? AND pickup_date <= ?)
                    OR (delivery_date IS NULL AND pickup_date IS NULL AND id IN (
            ...
            ps.setInt(1, driverId);
            ps.setString(2, Load.Status.BOOKED.name());
            ps.setString(3, Load.Status.CANCELLED.name());
            ps.setDate(4, java.sql.Date.valueOf(start));
            ps.setDate(5, java.sql.Date.valueOf(end));
            ps.setDate(6, java.sql.Date.valueOf(start));
            ps.setDate(7, java.sql.Date.valueOf(end));
            ps.setInt(8, driverId);
            ps.setString(9, Load.Status.DELIVERED.name());
            ps.setString(10, Load.Status.PAID.name());
            ps.setString(11, Load.Status.IN_TRANSIT.name());
    ...
    private Load extractLoad(ResultSet rs) throws SQLException {
        ...
        Load load = new Load(id, loadNumber, poNumber, customer, customer2, pickUp, drop, driver, truckUnitSnapshot, 
