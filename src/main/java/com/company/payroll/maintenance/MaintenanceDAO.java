...
public class MaintenanceDAO {
    ...
    public MaintenanceTab.MaintenanceSchedule saveSchedule(MaintenanceTab.MaintenanceSchedule schedule) throws DataAccessException {
        String sql = """
            INSERT INTO maintenance_schedules (
                vehicle, service, due_date, due_mileage, priority, notes,
                is_recurring, recurrence_interval_days, recurrence_interval_miles,
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL);
            ...
            pstmt.setString(1, schedule.getVehicle());
            pstmt.setString(2, schedule.getService());
            pstmt.setDate(3, Date.valueOf(schedule.getDueDate()));
            pstmt.setInt(4, schedule.getDueMileage());
            pstmt.setString(5, schedule.getPriority());
            pstmt.setString(6, null); // notes not in MaintenanceSchedule yet
            pstmt.setBoolean(7, false);
            pstmt.setNull(8, Types.INTEGER);
            pstmt.setNull(9, Types.INTEGER);
            pstmt.setString(10, "mgubran1");
            pstmt.setString(11, "mgubran1");
            
            pstmt.executeUpdate();
            logger.info("Created maintenance schedule for vehicle: {}", schedule.getVehicle());
            
            return schedule;
            
        } catch (SQLException e) {
            logger.error("Failed to save maintenance schedule", e);
            throw new DataAccessException("Failed to save maintenance schedule", e);
        }
    }
    
    public List<MaintenanceTab.MaintenanceSchedule> findAllSchedules() throws DataAccessException {
        String sql = "SELECT * FROM maintenance_schedules ORDER BY due_date ASC";
        List<MaintenanceTab.MaintenanceSchedule> schedules = new ArrayList<>();
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL);
            ...
            while (rs.next()) {
                MaintenanceTab.MaintenanceSchedule schedule = new MaintenanceTab.MaintenanceSchedule();
                schedule.setVehicle(rs.getString("vehicle"));
                schedule.setService(rs.getString("service"));
                schedule.setDueDate(rs.getDate("due_date").toLocalDate());
                schedule.setDueMileage(rs.getInt("due_mileage"));
                schedule.setPriority(rs.getString("priority"));
            ...
            logger.error("Failed to find all maintenance schedules", e);
            throw new DataAccessException("Failed to find maintenance schedules", e);
        }
    }
    
    public List<MaintenanceTab.MaintenanceSchedule> findOverdueSchedules() throws DataAccessException {
        String sql = "SELECT * FROM maintenance_schedules WHERE due_date < ? ORDER BY due_date ASC";
        List<MaintenanceTab.MaintenanceSchedule> schedules = new ArrayList<>();
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL);
            ...
            logger.error("Failed to find overdue maintenance schedules", e);
            throw new DataAccessException("Failed to find overdue schedules", e);
        }
    }
    
    // Statistics and Analytics
    
    public MaintenanceTab.MaintenanceData getMaintenanceData(LocalDate startDate, LocalDate endDate) throws DataAccessException {
        MaintenanceTab.MaintenanceData data = new MaintenanceTab.MaintenanceData();
        
        // Get records
        List<MaintenanceRecord> records = findByDateRange(startDate, endDate);
        
        // Convert to MaintenanceTab.MaintenanceRecord format
        List<MaintenanceTab.MaintenanceRecord> tabRecords = new ArrayList<>();
        for (MaintenanceRecord record : records) {
            MaintenanceTab.MaintenanceRecord tabRecord = new MaintenanceTab.MaintenanceRecord();
            tabRecord.setDate(record.getDate());
            tabRecord.setVehicle(record.getVehicle());
            tabRecord.setServiceType(record.getServiceType());
            tabRecord.setMileage(record.getMileage());
            tabRecord.setCost(record.getCost());
            tabRecord.setTechnician(record.getTechnician());
            tabRecord.setStatus(record.getStatus());
            tabRecord.setNotes(record.getNotes());
            tabRecords.add(tabRecord);
        }
        data.setRecords(tabRecords);
        
        // Get schedules
        data.setSchedules(findAllSchedules());
