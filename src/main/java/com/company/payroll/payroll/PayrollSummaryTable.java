...
public class PayrollSummaryTable extends VBox {
    ...
    private void configureColumns() {
        ...
        currentPercentagesCol.setCellValueFactory(p -> new SimpleStringProperty(
            String.format("D:%.0f%% C:%.0f%% S:%.0f%%", 
                p.getValue().driverPercent,
                p.getValue().companyPercent,
                p.getValue().serviceFeePercent)
        ));
        currentPercentagesCol.setPrefWidth(120);
        currentPercentagesCol.setMinWidth(110);
        currentPercentagesCol.setCellFactory(col -> new TableCell<>() {
            @Override
            protected void updateItem(String item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
                    ...
                    setText(item);
                    setStyle("-fx-font-size: 11px; -fx-text-fill: #757575;");
                    setAlignment(Pos.CENTER);
                }
            }
        });
        ...
        configuredPercentagesCol.setPrefWidth(120);
        configuredPercentagesCol.setMinWidth(110);
        configuredPercentagesCol.setCellFactory(col -> new TableCell<>() {
            @Override
            protected void updateItem(String item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null || item.isEmpty()) {
                    setText("-");
                    setStyle("-fx-font-size: 11px; -fx-text-fill: #BDBDBD; -fx-font-style: italic;");
                } else {
                    setText(item);
                    setStyle("-fx-font-size: 11px; -fx-text-fill: #1976D2; -fx-font-weight: bold;");
                    
                    // Add configured indicator
                    Label indicator = new Label("⚙️");
                    indicator.setTooltip(new Tooltip("Percentages were configured"));
                    setGraphic(indicator);
                    setContentDisplay(ContentDisplay.RIGHT);
                }
                setAlignment(Pos.CENTER);
        ...
        table.getColumns().setAll(java.util.List.of(
            ...
            grossCol,
            serviceFeeCol,
            currentPercentagesCol,
            configuredPercentagesCol,
    ...
    /**
     * Update the effective date and reload configured percentages
     */
