...
public class PayrollOtherAdjustmentPanel extends VBox {
    ...
    public PayrollOtherAdjustmentPanel(PayrollOtherAdjustments adjustmentsLogic, ComboBox<Employee> driverBox, DatePicker weekStartPicker) {
        ...
        getChildren().addAll(btnBox, adjustmentTable, new Label("Audit Trail for this week/driver:"), auditTable);
        ...
        adjustmentTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSel, newSel) -> {
            editBtn.setDisable(newSel == null || !canEdit(newSel));
            removeBtn.setDisable(newSel == null || !canEdit(newSel));
        });
    ...
    private void buildAuditTable() {
        ...
        tsCol.setCellValueFactory(cell -> new SimpleStringProperty(cell.getValue().timestamp.toString()));
        ...
        TableColumn<PayrollOtherAdjustments.AuditEntry, ?>[] columns = new TableColumn[] {
            tsCol, actionCol, detailsCol, byCol
        };
        auditTable.getColumns().setAll(columns);
    }
    ...
    private void addAdjustmentDialog() {
        ...
        if (result != null) {
            // Validation: no duplicate bonuses for same load
    ...
    private void editAdjustmentDialog() {
        ...
        if (result != null) {
            // Validation: no duplicate bonuses for same load (except for this adjustment)
            if (result.type.startsWith("Load Bonus:") && adjustmentRows.stream().anyMatch(a -> a.type.equals(result.type) && a.id != selected.id)) {
                showError("A bonus for this load already exists for this week.");
                return;
            }
            if (result.amount <= 0) {
